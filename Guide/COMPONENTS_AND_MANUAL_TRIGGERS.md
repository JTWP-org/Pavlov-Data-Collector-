...
# 1️⃣2️⃣ 🕵️ Player Connection Cross-Check
...
# 1️⃣9️⃣ 🧪 Manual Component Test Sequence
...
# 📋 Quick Manual Trigger Table
  ...
  🗺️ Mod.io enrichment R       un `collector.py`
  ...
  🖥️ ServerInfo trigger C      reate the configured `IN-serverinfo.json`
...
Need fresh public Pavlov server data from the shell?
    └── 🌍 update_pavlov_api.py

Need the ModKit to request fresh public Pavlov server data?
    └── 🌍 Create EXE_PPAPI.json in JTWP/Rcon
